...
class ObjectivesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create objective" do
    assert_difference('Objective.count') do
      post objectives_url, params: { objective: { name: @objective.name, position: @objective.position } }
    end
  ...
  test "should update objective" do
    patch objective_url(@objective), params: { objective: { name: @objective.name, position: @objective.position } }
